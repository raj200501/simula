...
| **Portfolio check** | After judging, code compares the SHIPs. Two that are the same idea (same screen, same reward, same kind of offer, or near-identical wording) would compete for one moment, so only the better-scored one ships. The other gets one revision to become different, or it's rejected. |
