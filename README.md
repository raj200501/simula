...
| "How do you know the judge is good?" | `npm run eval:judge` writes `out/<app>/proposals/judge-eval.md`: a single-fault confusion table (5 KB-precedent positives; each negative changes one field), showing whether code or the LLM caught each fault |
| **Rewarded flows** (Goal 4): state → mechanic → ad → value | `out/<app>/slides/deck.pdf` / `deck.html` / `png/`. Every SHIP gets a 5-frame flow slide (Today → What changed → Offer → Ad plays → Value received, with the trigger arrow and a "why" rail) plus a details slide (economics, KPI, holdout, SDK snippet) |
...
| Models | **Gemini:** `gemini-3.8-flash` for synthesis, HTML, QA fixes, proposals and judging; `gemini-3.5-flash-lite` for per-screen annotation. When a model is busy (503) or out of daily quota, the call moves to `gemini-3.5-flash` → `gemini-3.7-flash` → `gemini-3.6-flash` → Flash-Lite. **Claude:** `claude-opus-5` for everything |
...
- **Economics are ranges from public eCPM benchmarks** with a non-game haircut. The ARPDAU on the details slide is a labelled scenario, not a forecast.
